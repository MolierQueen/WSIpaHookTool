...
MONKEYDEV_PATH="${PROJECT_DIR}/WSIpaManager"
OPTOOL="${MONKEYDEV_PATH}/bin/optool"
WSIpaManager="${MONKEYDEV_PATH}/bin/WSIpaManager"
# temp path
...
function pack(){
	...
	if [[ ${MONKEYDEV_INSERT_DYLIB} == "YES" ]];then
		...
		"$WSIpaManager" inject -f -s "@executable_path/Frameworks/lib""${TARGET_NAME}""Dylib.dylib" -t "${BUILD_APP_PATH}/${APP_BINARY}"
